...
@click.option(
  "--strict", is_flag=True, help="Fail hard if embedding endpoints are unreachable"
)
@click.pass_context
def ingest(
  ctx: click.Context, source: str | None, skip_embeddings: bool, strict: bool
) -> None:
...
@click.option(
  ...
  help="Repository path to reindex",
)
@click.option(
  "--files", "-f", multiple=True, help="Specific files to reindex (can be repeated)"
)
...
@click.option(
  "--type", "chunk_types", help="Comma-separated chunk types (e.g. method,property)"
)
...
@click.option(
  "--rerank/--no-rerank", default=None, help="Enable/disable reranking (default: auto)"
)
@click.option(
  "--candidates",
  default=50,
  show_default=True,
  help="Candidates for pgvector before rerank",
)
@click.pass_context
def search(
  ctx: click.Context,
...
async def _search(
  ...
  chunk_types = (
    [t.strip() for t in chunk_types_str.split(",")] if chunk_types_str else None
  )
  ...
  for i, r in enumerate(results, 1):
    ...
    tag = (
      ...
      else (r.get("retrieval_tag", "") or f"[{retrieval}]")
    )
    ...
    if rerank_score is not None:
      meta = click.style(
        f"  [{chunk_type}] {src} {tag} rerank={rerank_score:.4f}", fg="cyan"
      )
    click.echo(header)
    click.echo(meta)
    if summary:
...
@cli.command()
@click.option("--group", "-g", help="Source group name to rebuild edges for")
@click.option("--all", "all_groups", is_flag=True, help="Rebuild edges for all groups")
@click.pass_context
def link(ctx: click.Context, group: str | None, all_groups: bool) -> None:
  """Rebuild cross-source graph edges."""
  if group and all_groups:
    raise click.UsageError("Cannot specify both --group and --all")
  if not group and not all_groups:
    raise click.UsageError("Specify --group <name> or --all")
  asyncio.run(_link(ctx.obj["config_path"], group, all_groups))


async def _link(config_path: str, group: str | None, all_groups: bool) -> None:
  from glyph.config import load_config as _load_config
  ...
  try:
    if group:
      count = await build_edges_for_group(store, group, cfg)
      click.echo(f"Built {count} edges for group '{group}'")
    elif all_groups:
      groups = {s.group for s in cfg.sources if s.group is not None}
      total = 0
      for g in sorted(groups):
        count = await build_edges_for_group(store, g, cfg)
        click.echo(f"Built {count} edges for group '{g}'")
        total += count
      click.echo(f"Total: {total} edges across {len(groups)} groups")
  finally:
    await store.close()
...
@click.option(
  "--port", "-p", default=8420, type=int, help="Port for SSE/HTTP transport"
)
@click.pass_context
